
<body>
<!-- Wrapper Starts -->
<div class="wrapper min_wrap">
    <div class="overlay"></div>
    <div class="side_bar"></div>

    <!-- Header Starts -->
    <header class="rc_navbar opaque-navbar">
        <div class="rc_navbar-brand">
            <a href="/"><img src="/images/favicon4.png" width="300px"></a>
                <div class="nav-icon"><span></span><span></span><span></span></div>
                <!--<div class="header-in">-->
            <nav>
                <ul>
                    <ul>
                        <!--<li>
                            <form>
                                <select class="select-dropdown button_no_border">
                                    <option selected>english</option>
                                    <option>hablo español</option>
                                    <option>我说中文</option>
                                    <option>tiếng việt</option>
                                    <option>나는 한국어 구사</option>
                                </select>
                            </form>
                        </li>-->
                        <li>
                            <form method="get" action="/partnerwithus">
                                <button class="btn rc_btn-outline-success my-2 my-sm-0 button_no_border" type="Submit">
                                    <i class="fa fa-users" aria-hidden="true"></i> partner with us</button>
                            </form>
                        </li>
                        <li>
                            <form method="get" action="/login">
                                <button class="btn rc_btn-outline-success my-2 my-sm-0 button_no_border" type="Submit">
                                    <i class="fa fa-lock" aria-hidden="true"></i> login</button>
                            </form>
                        </li>
                    </ul>
                    <div class="clear"></div>
            </nav>

                <div class="clear"></div>
        </div>
        <!--</div>-->
    </header>

